...
@section('sidebarHook')
	<nav>
		<ul class="sidebar-pagination">
			@if (is_null($preview))
				<li class="disabled"><a rel="preview" href="#" title="disabled"><span aria-hidden="true">Â«</span></a></li>
			@else
				<li><a rel="preview" href="{{ houseShowUrl($preview->slug) }}" title="{{ $preview->title }}">Â«</a></li>
			@endif
			@if (is_null($next))
				<li class="disabled"><a rel="preview" href="#" title="disabled"><span aria-hidden="true">Â»</span></a></li>
			@else
				<li><a rel="next" href="{{ houseShowUrl($next->slug) }}" title="{{ $next->title }}">Â»</a></li>
			@endif
		</ul>
	</nav>
...
@section('content')
	...
	<article class="article">
		...
		<section class="article-head">
			...
			<div class="row">
				<div class="col-md-8">
					<div class="article-head-price">GiÃ¡: {{ MoneyHelper::price($house->price, $house->money_unit, $house->sale_type) }}</div>
				</div>
				<div class="col-md-2">
		...
		<section class="article-detail">
			<header><h3 class="article-section-title">Chi tiáº¿t</h3></header>
			<ul class="row">
				<li class="col-md-4 active">Diá»‡n tÃ­ch sá»­ dá»¥ng: <span>{{ $house->m2 }}m2</span></li>
				<li class="col-md-4 @if (0 != $house->road) active @endif">
					Ä�Æ°á»�ng trÆ°á»›c nhÃ : @if (0 != $house->road) <span>{{ $house->road }}m2</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->common_room) active @endif">
					PhÃ²ng sinh hoáº¡t chung: @if (0 != $house->common_room) <span>{{ $house->common_room }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->toilet) active @endif">
					Sá»‘ phÃ²ng vá»‡ sinh: @if (0 != $house->toilet) <span>{{ $house->toilet }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->bedroom) active @endif">
					Sá»‘ phÃ²ng ngá»§: @if (0 != $house->bedroom) <span>{{ $house->bedroom }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->balcony) active @endif">
					Ban cÃ´ng: @if (0 != $house->balcony) <span>{{ $house->balcony }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->floors) active @endif">
					Sá»‘ táº§ng: @if (0 != $house->floors) <span>{{ $house->floors }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->kitchen) active @endif">
					Báº¿p: @if (0 != $house->kitchen) <span>{{ $house->kitchen }}</span> @endif
				</li>
				...
				<li class="col-md-4 @if (0 != $house->direction) active @endif">
					HÆ°á»›ng nhÃ : @if (0 != $house->direction) <span>{{ HouseDirectionOption::getLabel($house->direction) }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->living_room) active @endif">
					PhÃ²ng khÃ¡ch: @if (0 != $house->living_room) <span>{{ $house->living_room }}</span> @endif
				</li>
				<li class="col-md-4 @if (0 != $house->license) active @endif">
					TÃ¬nh tráº¡ng phÃ¡p lÃ½: @if (0 != $house->license) <span>{{ HouseLicenseOption::getLabel($house->license) }}</span> @endif
				</li>
			</ul>
		</section>

		<section class="article-feature">
			<header><h3 class="article-section-title">TÃ­nh nÄƒng</h3></header>
			<ul class="row">
	...
	<section class="relation">
		<header><h3 class="article-section-title">NhÃ  Ä‘áº¥t tÆ°Æ¡ng tá»±</h3></header>
